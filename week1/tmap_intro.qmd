...
```{r}
# Plotting a single spacial object
tm_shape(world_vector) + # Defines spatial object to plot
...
tm_shape(world_cities) + # Defines the second object to plot
  tm_dots() # Defines how to plot these 
```
...
tm_shape(world_elevation) + # Plotting global elevations 
  tm_raster() +
  tm_shape(world_vector) +
  tm_borders() + # Don't fill in because this is already done by raster data
  tm_shape(world_cities) +
  tm_dots() +
  tm_text("name", size = 0.7, col = "grey5")
```
...
```{r}
tm_shape(world_cities) + # Data associated with this is what will be used as the spatial extent, which is why this plot is so zoomed in
  tm_dots() +
  tm_text("name") +
...
tm_shape(world_elevation) +
  ...
  tm_shape(world_vector) +
  tm_borders() +
  tm_shape(world_cities, is.main = TRUE) + # Plotting cities last but set as main shape
  tm_dots() +
  tm_text("name")
# Q: is the utility in this that the layers are still in the correct order but the extent is customizable?
```

### Map extent- Spatial Extent
...
tm_shape(world_elevation, bbox = c(-15, 35, 45, 65)) + # CRS is in lat, lon x min, y min, x max, y max
  tm_raster() +
  tm_shape(world_vector) +
  tm_borders()
...
```{r}
tm_shape(world_elevation, bbox = "Europe") + # Europe is predefined as a spatial extent within tmap
  tm_raster() +
  tm_shape(world_vector) +
  tm_borders()
...
tm_shape(world_vector) +
  tm_polygons(fill = "green", col = "pink", lwd = 10)

```

We modify the colors using the `col` and `border.col` arguments and other arguments borrowed from `ggplot2`.
...
```{r}
# Fill polygon with a color palette representing a variable
tm_shape(world_vector) +
...
```{r}
tm_shape(world_cities) +
  tm_symbols(shape =22) # This is a square I guess

tm_shape(world_cities) +
  tm_dots() # Smaller circles
```
...
tm_shape(world_vector) +
  tm_polygons(fill = 'life_expectancy',
              fill.scale = tm_scale(values = c("orange", "blue")), 
              fill.legend = tm_legend(title = "Life Expectancy (years)", position = tm_pos_out("left"))) 
...
tm_shape(world_vector) +
  tm_polygons(fill = 'life_expectancy',
              ...
              # fill.scale = tm_scale(values = "Pastel1"),
              fill.scale = tm_scale(values = "Accent"),
              fill.legend = tm_legend(title = "Life Expectancy (years)", position = tm_pos_out("left"))) 
```
...
tm_shape(world_vector) +
  ...
  tm_shape(world_cities) +
  tm_symbols(
    size = "pop2020", #size is according to the population in 2020,
...
tm_shape(world_vector) +
  tm_fill() +
  tm_scalebar() +
  tm_compass(north = 0, # You can change the orientation of the compass if say you weren't making a colonialist map
             position = c("left", "top"))
```
...
tm_shape(world_vector) +
  tm_polygons(#fill = "wb_income_region", 
              fill = 'gdp_per_cap', 
              #fill.scale = tm_scale(breaks = c(0, 500, 1000, 2000, 4000, 8000)), 
...
map1 <- tm_shape(world_vector) +
  tm_polygons(fill = "gdp_per_cap", 
              fill.legend = tm_legend(title = "GDP Per Capita"))
tmap_save(map1, here::here("week1", "tmap_example.png"))
