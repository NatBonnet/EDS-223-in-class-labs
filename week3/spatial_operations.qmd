...
# Filter to Canterbury
canterbury <- nz %>% 
  filter(Name == "Canterbury")
```

```{r}
# Create tmap to look at Canterbury within NZ with elevation data
tm_shape(nz) +
  tm_polygons() +
  tm_shape(canterbury) +
  tm_polygons(fill = "darkgrey") +
  tm_shape(nz_height) +
  tm_dots(fill = "red") # 100 highest elevations in NZ
```
...
```{r}
# Subset heights for regions only within Canterbury
c_height1 <- nz_height[canterbury, ] # 70 observations of highest elevation that are within canterbury
...
```{r}
c_height_disjoint <- nz_height[canterbury, , op = st_disjoint] # rows, columns, parameter 
...
```{r}
# Subset to features in Canterbury
c_height2 <- nz_height %>% 
  st_filter(y = canterbury, .predicate = st_intersects) # Why use .predicate instead of op?
```
...
```{r}
# Sparse binary predicate lst- either empty or 1
nz_height_sgbp <- st_intersects(x = nz_height, y = canterbury)
nz_height_sgbp # Returns empty if geometry intersects there, otherwise returns 1
...
map2 <- tm_shape(nz) +
  ...
  tm_shape(c_height2) +
  tm_dots(fill = "red") +
  tm_title(text = "st_filter subsetting")
...
map3 <- tm_shape(nz) +
  ...
  tm_shape(c_height3) +
  tm_dots(fill = "red") +
  tm_title(text = "st_intersects subsetting")
...
# st_is_within_distance returns empty or 1
nz_height_logical <- st_is_within_distance(nz_height, canterbury, dist = units::set_units(1000, "km"), 
                                           sparse = FALSE) # Return as logical vector
...
tm_shape(nz) +
  ...
  tm_shape(c_height4) +
  tm_dots(fill  = "red") +
  tm_title(text = "Highest elevations within Canterbury and within 100 km of canterbury polygon")
```
...
```{r}
joined_df <- left_join(df_A, by = c("id_A" = "id_B"))
...
nz_height_left_join %>% 
  ...
  st_drop_geometry()
# Number of highest elevation data in dataset from nz_height
```
...
cycle_hire_osm <- spData::cycle_hire_osm # GIves capacity data on stations, want to join to include these
```
...
# Some stations in cycle_hire had multiple matches in osm, so the output observation length is different than the left dataset
```
...
```{r}
# aggregate values for single points in cycle_hire)join
...
```{r}
# Select just the value we want to aggregate
incongruent2 <- incongruent %>% 
  select(value)

incongruent2

# Use area-weighted inerpolation to aggregate the "value" attribute

aggregating_zones_area_weighted <- st_interpolate_aw(x = incongruent2, 
                                                    aggregating_zones, 
                                                    extensive = TRUE) # pop or count value splitting area proportionally by the area- if ratio or density you'd set to false

aggregating_zones_area_weighted$value # two values represnting area for red value and area for the other
```
...
# Summarize the total population within eahc region
...
```{r}
us_west <- us_states[us_states$REGIONS == "West", ]
...
```{r}
seine <- spData::seine
seine_buffer_5km <- st_buffer(seine, dist = units::set_units(5, "km")) # Expanding buffer around to
seine_buffer_50km <- st_buffer(seine, dist = units::set_units(50, "km"))
```
...
```{r}
nz_centroid <- st_centroid(nz) # Finds the center of the geometry no matter what it is looking at ex: donut, it would be in the hole not in the object
seine_centroid <- st_centroid(seine)

nz_pos <- st_point_on_surface(nz)
seine_pos <- st_point_on_surface(seine) # ALSO finds the centroid but puts it on the object- might be a little off but closer to what you want
```
