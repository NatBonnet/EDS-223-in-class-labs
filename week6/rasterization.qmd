...
- **If it is too low** (cell size is too large), the result may miss the full geographic variability of the vector data- you may be ignoring important data that is there from an environmental justice framework
...
# Add warning message to check crs and transform
if(crs(elevation) == crs(boundary)){
  print("Coordinate reference systems match")
} else{
  warning("Updating crs to match")
  # Transform data to match
  st_transform(boundary, crs = st_crs(elevation))
}
```
...
```{r}
# Update name to clarify this is a polygon
park_polygon <- boundary

# Create line version of park boundary
park_border <- boundary %>% 
  st_cast(., "MULTILINESTRING") # If multiple lines that are unconnected they will be included
...
tmap_arrange(map1, map2, nrow = 1) # Fill varies
```
...
template_raster_high <- terra::rast(ext(park_polygon), resolution = 0.01, # Smaller cells =  better resolution
                        crs = st_crs(park_polygon))
...
template_raster <- rast(ext(cycle_hire_osm_projected), resolution = 1000, 
...
```{r}
# Count number of observations within each cell
...
# True color imagery- r, g, b in proper channels
map1 <- tm_shape(L7) +
  tm_rgb(col = tm_vars(c(3,2,1), multivariate = TRUE)) +
  tm_title(text = "true color")

# False color- show near infrared in red channel with red and green- good for gauging plant health
map2 <- tm_shape(L7) +
  tm_rgb(col = tm_vars(c(4,3,2),  multivariate = TRUE)) +
  tm_title(text = "NIR, red, green")

# False color- short wave infrared and near with green: makes water stand out so good for flood data
map3 <- tm_shape(L7) +
...
# Update names of layers to say NDVI instead of number
...
tmap_arrange(map1, map2, ndvi_map) # Displays the densest vegetation at highest scale values
```
...
tmap_arrange(map1, map3, map_ndwi) # Anything between 0.1 and 1 is likely water 
```

Normalized Difference moisture index: vegetation index used in remote sensing to measure and monitor the water content in veg

NDMI = formula(NIR - SWIR) / (NIR + SWIR)

measuring moisture in plant levels: higher values is more moisture content
```{r}
ndmifunc <- function(nir, swir){
  (nir - swir)/ (nir +swir)
}

ndmi_rast <- lapp(L7[[c(4, 5)]], fun = ndmifunc)
names(ndmi_rast) <- "NDMI"

map_ndmi <- tm_shape(ndmi_rast) +
  tm_raster(
    col.scale = tm_scale_continuous(values = "brewer.greens"), 
    col.legend = tm_legend(tite = "NDMI", 
                           orientation = "portrait")) +
      tm_title(text = "NDMI")
    
tmap_arrange(map1, map_ndmi)
```
