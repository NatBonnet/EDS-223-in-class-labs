...
filelist <- list.files(here::here("week8", "data", "landsat-data"), full.names = TRUE)

# read in and store as a raster stack
landsat <- rast(filelist)

# Update layer names to match bands
...
SB_county_south <- st_read(here::here("week8", "data", "SB_county_south.shp")) %>%
...
```{r include=TRUE}
# Crop landsat scene to the extent to the SB county shapefile
landsat_crop <- crop(landsat, SB_county_south) # This creates a box using max and min- we now need to mask to turn values outside boundary but within box to be NA

# Mask raster to southern portion of santa barbara county
landsat_mask <- mask(landsat_crop, SB_county_south)

# Remove unnecessary things from env
rm(landsat, SB_county_south, landsat_crop)

# Plot again as RGB 
plotRGB(landsat_mask, r =3, g = 2, b=1, stretch = "lin")
```
...
require labeled data to train the model on
...
training_data <- st_read(here::here("week8", "data", "trainingdata.shp")) %>%
  st_transform(., crs = crs(landsat))
```

```{r}
# Visualize basic training data
tm_shape(training_data) +
...
```{r}
# Extract pixel values from landsat raster at the locations in the training data
training_data_values <- terra::extract(landsat, training_data, df =TRUE) # return as df instead of matrix
...
# Inspect levels of traing data to understand order of classes in prediction
...
dt_plot <- tm_shape(SB_classification_dt) +
  tm_raster(col.scale = tm_scale(values = c("#8DB580", "#F2DDA4", "#7E8987", "#6A8EAE"), 
...
```{r}
# Convert raster to df and scale
landsat_df <- as.data.frame(landsat, xy = TRUE, na.rm = TRUE)

# Extract spectral bands for clustering
spectral_df <- landsat_df[,c("blue", "green", "red", "NIR", "SWIR1", "SWIR2")]

# Scale the data
spectral_data_scaled <- scale(spectral_df)
```
...
sb_classification_kmeans <- rast(landsat[[1]]) # Copy landsat layer 
values(sb_classification_kmeans) <- NA # Assign all values in the raster NA

# Take xy coords from landsat_df and convert them to cell numbers for the raster
cells <- cellFromXY(sb_classification_kmeans, landsat_df[, c("x", "y")])

# Update cells with values from cluster data column
values(sb_classification_kmeans)[cells] <- landsat_df$cluster

# Look at cluster characteristics to determine landcover
landsat_df %>% 
  group_by(cluster) %>% 
  summarise(
    blue = mean(blue),
    ...
    SWIR2 = mean(SWIR2)) %>% 
arrange(cluster) %>% 
  knitr::kable(digits = 2, caption = "mean spectral values by cluster")

# High NIR low red: 3
# Higher blue and low NIR: 4
# Higher values across all: 1
# Moderate values: 2

# Set category labels
levels(sb_classification_kmeans) <- data.frame(
  ID = 1:4, 
  landcover = c("soil/dead grass", 
                "urban", 
                "green vegetation", 
                "water")
)
