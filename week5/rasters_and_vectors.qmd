...
```{r}
# Will corp to the extent of bounding box- not a perfect crop so need to mask
...
# Cells outside vector are converted to NA
...
```{r}
# Combine mask and crop to reduce everything outside the bonudary as much as possible
...
# Convert high elevation values into polygons and transform into a sf
elevation_mask_poly <- as.polygons(elevation_masked) %>% 
  st_as_sf()
```
...
map1 <- tm_shape(elevation_masked) +
  tm_raster() +
...
```{r}
bryce = raster("week5/data/Bryce_Canyon_GeoTIFF/Bryce_Canyon.tif")

# Transform the spatial structure of raster into a matrix
bryce_mat = raster_to_matrix(bryce)

# Reduce the size y 1/4th for faster processing
...
```{r}
# Make colors to elevation values
bryce_small %>% 
  height_shade() %>%  
  plot_map() # Plot map
```
...
```{r}
# Define bbox for area of interest
...
```{r}
# Create extent object from bbox coords
extent_zoomed = extent(utm_bbox[1], utm_bbox[2], utm_bbox[3], utm_bbox[4])

# Crop full Bryce raster to our area of interest
bryce_zoom = crop(bryce, extent_zoomed)

# Convert cropped raster to a matrix for rayshader 
...
# Query open street map(osm) to pull out lines as highways
...
```{r}
# Combine trails layer with different styles for each kind of path
...
```{r}
# Add overlays of streams, trails, and popular points
base_map %>% 
...
# Create 3d map of specified window size
plot_3d(bryce_zoom_mat, windowsize=c(1200,800))
render_camera(theta=240,  phi=30, zoom=0.3,  fov=60) # How wide or narrow do we want that initial iamge to be
