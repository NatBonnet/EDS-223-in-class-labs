...
```{r}
# Create st_point with lat and lon for london
...
```{r}
# Create a df of attributes about London
...
```{r}
# Load data frame included in spData
world <- spData::world
class(world)

# Look at dimensions (rows, cols)
dim(world)
...
```{r}
# Summary stats for a specific column
...
```{r}
# Need to explicitly use an st function to get rid of the geometry column

# This does not work
world_df <- world %>% 
  select(-geom)

colnames(world_df) # Still there
```
...
```{r include=TRUE}
# Using an st function
world_df <- st_drop_geometry(world)

colnames(world_df)

class(world_df) # no more sf in the list
```
...
```{r}
london_sf_transform <- st_transform(london_sf, crs = st_crs(london_proj)) # You can set the crs to be the same by referencing the crs of another object
```
...
```{r}
# Example of making a check 
if (st_crs(dataset1) == st_crs(dataset2)){
  warning("crs do not match") # If returning this warning make a transformation to match them
...
# Return projection and what parameters are being used 
...
tmap_arrange(display_crs, transformed_crs) # In one we transformed our data, the other we just set the crs within the map building 
```
...
```{r}
nrow(coffee_data) # Does not contain spatial information
nrow(world) # We want to combine to see how much coffee prouction varies by country
```
...
tm_shape(world_coffee) +
  tm_fill(fill = "coffee_production_2017", 
          fill.legend = tm_legend(title = "World Coffee Production in 2017"))
st_crs(world_coffee) # Not projected so how did we plot this in 2d? This stretches 
```
