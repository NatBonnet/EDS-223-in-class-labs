...
```{r}
# Crop surface change to the extent of the Eaton perimeter
...
# Remove values outside upper and lower quartile
...
tm_raster(
  col.scale = tm_scale_intervals(values = fire_palette, 
...
# Crop difference rasters to Altadena extent
...
```{r}
# Rayshader takes matrices rather than rasters
...
overlay <- height_shade(
  ...
  texture = colorRampPalette(c("#8B0000", "#FF6347", "#FFFFFF", 
                  "#87CEEB", "#00008B"))(256), # Create 256 levels of brightness
  range = c(-20, 20) # Between these values create 256 color options
)

# Create map with hillshade
elev_mat %>% 
  # Create a black and white hillshade effect
  sphere_shade(texture = "bw",  sunangle = 315) %>% 
  add_overlay(overlay, alphalayer = 0.7) %>% 
  # add realistic shadows at 50% intensity to enhave the 3d effect
...
# Extract the minimum elevation loss for each building buffer
...
```{r}
ggplot(damage_plot, aes(x = age_category, y = elevation_loss, fill = age_category))+
  geom_boxplot(alpha = 0.7) +
  scale_fill_brewer(palette = "PuBuGn") +
  coord_flip() +
  labs(title = "Building Time Period vs. Elevation Loss from Eaton Fire", 
       x = "Construction Date", 
       y = "Elevation Loss (m)") +
  theme_minimal() +
