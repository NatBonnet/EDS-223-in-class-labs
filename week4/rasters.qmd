...
# Spatial raster is unique to terra
```

```{r}
# Use tmap to look at data
...
# Find out the number of layers in our raster object
nlyr(landsat)

# Find out the object names
names(landsat)
```
...
# Outputs 1 raster layer- need to be in the same resolution (similar to CRS)
```


```{r}
# Map austria, switzerland, then merged
...
map3 <- tm_shape(merged) +
  tm_raster(col.legend = tm_legend("Elevation (m)")) +
  tm_title(text = "Austria and Switzerland Merged")
...
```{r}
# Using a sample: not every single data point
...
```{r}
# Tells us how many cells are being counted as what
freq(zion_land)
...
```{r}
# Pick row 1, col 1 (top left value)
zion_elevation[1,1]
...
# Find the same thing using row, column indexing
zion_elevation[2, 3]
```
...
```{r}
# Cell ids for multiple raster layers
landsat[1] # first row value for all cols
```
...
```{r}
# Still need the same number of columns overwriting even if replacing 1 number
...
# Check NA values
global(is.na(test_raster), "sum") # Sum gives total number within the raster

# Assign elevation of less than 20 to NA
test_raster[test_raster < 20] <- NA

# Check NA values
global(is.na(test_raster), "sum") # 1 value now that was less than 20 meters in zion raster
```
...
```{r}
# Create a point within area covered by raster
point <- matrix(c(-113, 37.5), ncol = 2)

# Extract coordinate using cell id- returns the cell id itself
id <- cellFromXY(zion_elevation, xy = point)

# Index the cell- returns value within the id'ed cell
zion_elevation[id]


# Aproach 2: extract
# Extract raster value at a point
terra::extract(zion_elevation, point) # Finds that coordinate point and just returns the value without finding col id
```
...
```{r}
# Find the extent of the current raster using terra::ext
terra::ext(zion_elevation) # Returns a bbox essentially

# Create a raster with a smaller extent
clip <- rast(xmin = -113.2, xmax =-113, ymin = 37.2, ymax = 37.9, 
             resolution = 0.3, # Each cell is 0.3x0.2 degrees
             vals = 1)

# Select the values that fall within the clipped extent
zion_elevation_clip <- zion_elevation[clip]

# Verify that output has fewer values than the original
...
```{r}
# Specify drop = false to not drop the geometry-  keep as a raster
...
```{r}
# Build a check to make sure the extent cahnged by clipping the original data frame
...
```{r}
# Create a raster mask of the same resolution and ext
rmask <- zion_elevation

# Set all cells that have elevation less than 2000 meters to NA
rmask[rmask < 2000] <- NA
...
# Approach 2: using mask() function
...
```{r}
# Map original vs newly masked spatRasters
map1 <- tm_shape(zion_elevation) +
  tm_raster(col.legend = tm_legend(title = "Elevation (m)")) +
  tm_title(text = "Original")
...
map2 <- tm_shape(masked1) +
  tm_raster(col.legend = tm_legend(title = "Elevation (m)")) +
  tm_title(text = "subset mask")

map3 <- tm_shape(masked2) +
  tm_raster(col.legend = tm_legend(title = "Elevation (m)")) +
  tm_title(text = "masked function")
...
# Use reclass matrix to reclassify elevation raster- aggregates each value into factor group
reclass <- classify(zion_elevation, rcl = rcl)
values(reclass) <- as.factor(values(reclass))
...
```{r}
# Taking 9x9 box and assigning the minimum value in the vbox to the whole thing
...
# We can see that this essentially just decreases the resolution
```
...
```{r}
zonal(zion_elevation, reclass, fun = "mean") %>%
...
# Geometry Operations- change geometries (either extent, origin, or resolution)
...
```{r}
# Add 1 row of cells to bottom and top of raster, and 200 columns to the left and right
...
```{r}
# Mismatch in the number of cells so we cannot add them
...
```{r}
# Check the origin- returns coordinates 
origin(zion_elevation)
# Are we currently assuming a speficic origin reference, I've seen that bottom left is pretty typical but why can we assume that other than having previous knowledge about where we are looking at
```
...
```{r}
# Coarsen resolution by a factor of 5 - take average of 25 cells 
...
zion_elevation_disagg <- disagg(zion_elevation_coarse, fact =5, method = "bilinear") # Going from 1 pixel to 5x5

# Create check to see if these are the same
